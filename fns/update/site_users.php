...
if ($force_request || role(['permissions' => ['site_users' => 'edit_users', 'profile' => 'edit_profile'], 'condition' => 'OR'])) {
    ...
    if ($noerror && !empty($user_id)) {
        ...
        if (isset($data['location_id']) && !empty($data['location_id'])) {
            ...
            $where = ["geo_id" => $data['location_id']];
            $location = DB::connect()->select("site_location", $columns,$where);
            $update_data["geo_latitude"] = $location['0']['latitude'];
            $update_data["geo_longitude"] = $location['0']['longitude'];
