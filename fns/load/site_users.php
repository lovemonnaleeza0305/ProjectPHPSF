...
if (role(['permissions' => ['site_users' => ['view_site_users', 'view_online_users']], 'condition' => 'OR'])) {
    ...
    $columns = [
        ...
        'site_users.online_status', 'site_users.geo_longitude', 'site_users.geo_latitude',
    ];
    ...
    if (isset($private_data["online"]) || isset($private_data["guest_users"])) {
        

        if (!empty($user_latitude) && !empty($user_longitude)) {
            // $where["ORDER"] = Medoo::raw('ABS(geo_latitude-'.$user_latitude.') + ABS(geo_longitude - '.$user_longitude.') ASC');
    ...
    foreach ($site_users as $user) {
        ...
        if (isset($private_data["online"])) {
            ...
            if (!empty($user['geo_latitude']) && !empty($user['geo_longitude'])) {
                $output['content'][$i]->subtitle = calculate_distance($user_latitude, $user_longitude, $user['geo_latitude'], $user['geo_longitude']);
            }
            ...
            if (isset($permission['edit_users']) && $data["sortby"] === 'name_asc' || isset($permission['edit_users']) && $data["sortby"] === 'name_desc') {
                ...
                // display distans between all users.
                if (!empty($user['geo_latitude']) && !empty($user['geo_longitude'])) {
                    $output['content'][$i]->subtitle = calculate_distance($user_latitude, $user_longitude, $user['geo_latitude'], $user['geo_longitude']);
                }
            }
