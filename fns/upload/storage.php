                ...
                if (isset($files['files'])) {
                    foreach ($files['files'] as $index => $file) {
                        ...
                        if (in_array($file_type, $image_file_formats)) {
                            ...
                            $quality = 10;
                            $imagick = new \Imagick(realpath($file['file']));
                            ...
                            if($width > 800) {
                                ...
                                $imagick->cropImage (800,800,0,0);
                            } else{
                                 $imagick->compositeImage($imagick, \imagick::COMPOSITE_ATOP, 0, 0);
                            }
                            ...
                            unlink($file['file']);
                            file_put_contents($file['file'],$imagick);
                            $attachment_type = 'image_files';
                            $insert_data = [
                                "user_id" => $user_id,
                                "file_name" => basename($file['file']),
                                ...
                                "thumbnail" => 'assets/files/storage/'.$user_id.'/thumbnails/'.basename($file['file']),
                                "attachment_type" => "image_files",
                                "is_show" => 0,
                                "created_on" => Registry::load('current_user')->time_stamp,
                                "updated_on" => Registry::load('current_user')->time_stamp,
                            ];
                            DB::connect()->insert("file_attachments", $insert_data);
                            $resize = [
                                'resize' => $file['file'],
                                'width' => 250,
                                'crop' => true,
