...
if (role(['permissions' => ['site_users' => ['create_user', 'edit_users'], 'profile' => 'edit_profile'], 'condition' => 'OR'])) {
    ...
    foreach ($custom_fields as $custom_field) {
        ...
        if ($custom_field['field_type'] === 'short_text' || $custom_field['field_type'] === 'link') {
            ...
            $form['fields']->$field_name = [
                "title" => Registry::load('strings')->$field_name, "tag" => 'input', "type" => 'text', "class" => 'field '.$field_name,
    ...
    if (!empty($user_id)) {
        ...
        $columns = $where = $join = null;
        $columns = [
            'geo_id', 'postal_code', 'residence', 'province', 'latitude', 'longitude',
        ];
        // $where["LIMIT"] = 5;
        $datas = array();
        $geo_datas = DB::connect()->select('site_location', $columns,$where );
        foreach($geo_datas as $geo_data){
            $data = array(
                'geo_id'    => $geo_data['geo_id'],
                'value'     => $geo_data['residence'].",".$geo_data['province'].",".$geo_data['postal_code'],
            );
            $datas[] = $data;
        }
        $form['location'] = $datas;
        ...
        file_put_contents($jsonpath.'locations.json', json_encode($datas));
    }
}
?>
