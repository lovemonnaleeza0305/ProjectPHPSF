...
$columns = [
    ...
    'site_users.geo_id', 'site_users.geo_latitude', 'site_users.geo_longitude', 'site_users.last_seen_on', 'login_sessions.log_device',
];
$where = [
...
if (isset($get_current_user_info[0])) {
    ...
    $join = [
        "[>]site_location(location)" => ["site_users.geo_id" => "geo_id"]
    ];
    ...
    $where = [
        "site_users.user_id" => $get_current_user_info[0]['id'],
        'LIMIT' => 1
    ];
    $get_location_info = DB::connect()->select('site_users', $join, $columns, $where);

    $get_current_user_info[0] =array_merge($get_current_user_info[0], $get_location_info[0]);
}
...
if (isset($get_current_user_info[0])) {
    if (isset($get_current_user_info[0]['id']) && !empty($get_current_user_info[0]['id'])) {
        ...
        if ($geoip_service) {
            if (empty($current_user_info->time_zone) || empty($current_user_info->country_code) || empty($current_user_info->geo_latitude)) {
                if (empty($current_user_info->time_zone) || empty($current_user_info->country_code) || empty($current_user_info->geo_latitude)) {
                ...
                if (empty($current_user_info->geo_latitude)) {
                    $data = array();
                    $data['user_id'] = $get_current_user_info[0]['id'];

                    if (!isset($geoplugin->latitude) || empty($geoplugin->latitude)) {
                        $geoplugin->latitude = '51.4964';
                    }

                    if (!isset($geoplugin->longitude) || empty($geoplugin->longitude)) {
                        $geoplugin->longitude = '-0.1224';
                    }
                    //This section Auto GeoLocation latitude and longitute!

                    // $current_user_info->geo_latitude = $data['geo_latitude'] = $geoplugin->latitude;
                    // $current_user_info->geo_longitude = $data['geo_longitude'] = $geoplugin->longitude;
                    //
                    // var_dump($get_current_user_info[0]['geo_latitude'], $get_current_user_info[0]['geo_longitude']);die;
                    $current_user_info->geo_latitude = $data['geo_latitude'] = $get_current_user_info[0]['latitude'];
                    $current_user_info->geo_longitude = $data['geo_longitude'] = $get_current_user_info[0]['longitude'];
                    $data['updated_on'] = $current_time_stamp;
                    DB::connect()->update('site_users', $data, ['user_id' => $data['user_id']]);
                }
...
// var_dump($current_user_info);die;
