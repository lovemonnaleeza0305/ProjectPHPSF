...
if ($force_request || role(['permissions' => ['site_users' => 'create_user']])) {
    ...
    $validate_custom_fields = true;//if guest false!!!
}
...
if ($create_user) {
    ...
    $result['error_variables'] = [];
    $columns = $where = null;
    $columns = ['custom_fields.field_id', 'custom_fields.string_constant(field_name)', 'custom_fields.field_type', 'custom_fields.required' , 'custom_fields.show_on_signup'];
    ...
    if ($validate_custom_fields) {
        foreach ($custom_fields as $custom_field) {
            if ((int)$custom_field['show_on_signup'] === 1) {
                $required_fields[] = $custom_field['field_name'];
                $custom_field_name = $custom_field['field_name'];
                if (isset($data[0][$custom_field_name])) {
                    $data[$custom_field_name] = $data[0][$custom_field_name];
                } else {
                    $data[$custom_field_name] = $data[$custom_field_name];
                }
            }
        }
    }
    ...
    if(isset($data['location_id'])&&($noerror)) {
        if((int) $data['location_id'] === 0 ){
            $result['error_message'] = Registry::load('strings')->location_doesnt_exist;
            $result['error_key'] = 'location_doesnt_exist';
            $noerror = false;
        }
    }
    if (!$force_request) {
