...
var asideload, asideappend = 0, asideskiptitle = 0;
var grid = false;
var gridshownum = 3;
var user_id = 0;
...
function load_aside(load, append = 0, skiptitle = 0,isgrid = false) {
    ...
    console.log(load,append,skiptitle);
    if (load.attr('load') !== undefined) {
        ...
        load_aside_request = $.ajax({
            ...
            if (isJSON(data)) {
                ...
                if(data.loaded!== undefined){
                    if(data.loaded.title =="Online") {
                        $('.main .aside > .site_records > .current_record > .button').removeClass('d-none');
                        $('.main .aside > .site_records > .current_record > .button').html('<span type = "button" class="list-button">List</span><span type = "button" class="grid-button">Grid</span>');
                    } else if((data.loaded.title =="Users")||(data.loaded.title =="Alle gebruikers")) {
                        $('.main .aside > .site_records > .current_record > .button').removeClass('d-none');
                        $('.main .aside > .site_records > .current_record > .button').html('<span type = "button" class="list-button">List</span><span type = "button" class="grid-button">Grid</span>');
                    } else if(data.loaded.title =="Storage"||(data.loaded.title =="Opslag")) {
                        $('.main .aside > .site_records > .current_record > .button').removeClass('d-none');
                        $('.main .aside > .site_records > .current_record > .button').html('<span type = "button" class="list-button">List</span><span type = "button" class="grid-button">Grid</span>');
                    } else {
                        $('.main .aside > .site_records > .current_record > .button').addClass('d-none');
                    }
                }
                if (!preloader_disabled) {
                ...
                if (totalitems > 0) {
                    $.each(content, function(key, val) {
                        ...
                        list = list + '</div>';
                        if($('.main .aside > .site_records > .records > .list').hasClass('grid')){
                            list = list + '<div class="center d-none">';
                        } else {
...
$('body').on('click', ".main .aside > .site_records > .records > .list > li > div > .left-grid > .options", function(e) {
    if ($(this).find("span").is(":visible")) {
        $(".main .aside > .site_records > .records > .list > li > div > .left-grid > .options > span").hide();
    } else {
        $(".main .aside > .site_records > .records > .list > li > div > .left-grid > .options > span").hide();
